...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        new_data = data[data['Launch Site']== entered_site]
        fig = px.pie(new_data, names='class', title="Launch Site "+ entered_site)
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site, mass):
    data = spacex_df[(spacex_df['Payload Mass (kg)'] >= mass[0]) & (spacex_df['Payload Mass (kg)'] <= mass[1])]
    if entered_site == 'ALL':
        ...
        # return the outcomes piechart for a selected site
        new_data = data[data['Launch Site'] == entered_site]
        fig = px.scatter(new_data, x='Payload Mass (kg)', y='class', color="Booster Version Category")
        return fig
